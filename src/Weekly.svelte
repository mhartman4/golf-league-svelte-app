...
<script>
  import { onMount } from "svelte"
			  ...
			  if (team.gsx$roster.$t != undefined) {
					JSON.parse(team.gsx$roster.$t).forEach((player) => {
						...
						if (pgaPlayerMatches.length > 0) {
							...
							player.proj_money = numeral(pgaPlayer.rankings.projected_money_event),
							player.today = pgaPlayer.today,
							player.thru = pgaPlayer.thru,
							player.total = pgaPlayer.total,
							player.player_id = pgaPlayer.player_id
	...
	const getRelevantTournament = async () => {
		...
		const tourneysBeforeToday = await data.feed.entry.filter(event => new Date(Date.parse(event.gsx$date.$t)) <= today.setHours(0,0,0,0))
		const tourneyId = await tourneysBeforeToday.slice(-1)[0].gsx$tournamentid.$t
  ...
  const doTheMath = (pgaStandings, teamRosters) => {
		console.log("hello")
	}
